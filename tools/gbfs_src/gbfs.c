...
"usage: gbfs <option> ARCHIVE [FILE...]\n"
"option: -n |> remove file extension\n";
...
int main(int argc, char **argv)
{
  ...
  unsigned int current_arg = 2;
  unsigned int no_extension = 0;
  unsigned int output_arg = 1;
  ...
  if(argc <= 2 || (no_extension == 1 && argc <= 3))
  {
    fputs(help_text, stderr);
    return 1;
  }
  ...
  header.total_len = header.dir_off + (argc - current_arg) * sizeof(GBFS_ENTRY);
  ...
  while(current_arg < argc)
  {
    ...
    infile = fopen(argv[current_arg], "rb");
    if(!infile)
    {
      fprintf(stderr,
              "could not open %s: %s", argv[current_arg], strerror(errno));
      fclose(outfile);
      remove("gbfs.$$$");
      free(entries);
      return 1;
    ...
    if (no_extension == 0)
    {
      strncpy(entries[n_entries].name, basename(argv[current_arg]), sizeof(entries[n_entries].name));
    }
    else
    {
      char* _file_name = remove_extension(basename(argv[current_arg]));
      strncpy(entries[n_entries].name, _file_name, sizeof(entries[n_entries].name));
      free(_file_name);
    }
  ...
  remove(argv[output_arg]);  /* some systems don't auto-remove the rename target */
  if(rename("gbfs.$$$", argv[output_arg]))
  {
